...
import (
	...
	"greenlight.sanjayojha.github/internal/validator"
)
...
func (app *application) readJSON(w http.ResponseWriter, r *http.Request, dst any) error {
	r.Body = http.MaxBytesReader(w, r.Body, 1_048_576) // 1MB (bytes)
	...
	if err != nil {
		...
		case errors.As(err, &unmarshalTypeError):
			...
			return fmt.Errorf("request body contains badly-formed JSON (at character %d)", unmarshalTypeError.Offset)
		...
		case strings.HasPrefix(err.Error(), "json: unknown field "):
			...
			return fmt.Errorf("request body contains unknown JSON key %s", fieldName)
